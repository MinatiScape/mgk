...
static void chipone_panel_init(struct chipone_lcm *ctx)
{
    ...
    chipone_dcs_write_seq_static(ctx,0x48,0x00); //切频 60hz-command
    chipone_dcs_write_seq_static(ctx,0x53,0xE8);
    chipone_dcs_write_seq_static(ctx,0x35,0x00);
	chipone_dcs_write_seq_static(ctx,0x11);
	msleep(120);
	...
	chipone_dcs_write_seq_static(ctx,0x29);
	msleep(50);
	...
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		chipone_dcs_write_seq_static(ctx, 0x48, 0x20);//drv-The producer of Ying screen recorded two different frame rates, causing some screens to cut 120 screen.-pzp
		//msleep(40);
	...
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		chipone_dcs_write_seq_static(ctx, 0x48, 0x10);
		//msleep(40);
	...
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		chipone_dcs_write_seq_static(ctx, 0x48, 0x00);
		//msleep(40);
		pr_info("%s:%d  60 display_mode end\n", __func__, __LINE__);
	}
}
	...
	void *dsi, dcs_write_gce cb, void *handle)
{
	...
	/*Enter AOD 15nit*/
	chipone_dcs_write_seq_static(ctx, 0x51, 0x01, 0x55);
...
static int chipone_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_info("%s+ chipone,hc5622,cmd,120hz\n", __func__);
...
static const struct of_device_id chipone_of_match[] = {
	{
	 .compatible = "chipone,hc5622,cmd,120hz",
	},
	{}
};
