...
int husb311_fault_status_clear(struct tcpc_device *tcpc, uint8_t status)
{
	if (status & TCPC_V10_REG_FAULT_STATUS_VCONN_OV)
		husb311_fault_status_vconn_ov(tcpc);
...
static int husb311_tcpc_deinit(struct tcpc_device *tcpc)
{
	...
	// drv add tankaikun, fix power off charging cannot work,20231204 start
	msleep(10);
	husb311_set_cc(tcpc, TYPEC_CC_RD);
	// drv add tankaikun, fix power off charging cannot work,20231204 end
