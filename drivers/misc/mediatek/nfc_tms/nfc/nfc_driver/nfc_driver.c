...
/*  for clock buffer */
#include "../../../clkbuf/v1/inc/mtk_clkbuf_ctl.h"
...
int nfc_device_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    TMS_WARN("NFC driver version is %06x-%s-%s\n", LINUX_VERSION_CODE,
             DRIVER_VERSION, NFC_VERSION);
...
#if defined(CONFIG_TMS_NFC_DEVICE_MODULE)
module_i2c_driver(nfc_i2c_driver);
#else
static int __init nfc_driver_init(void)
{
    int ret;
...
static void __exit nfc_driver_exit(void)
{
    TMS_INFO("Unloading nfc driver\n");
    i2c_del_driver(&nfc_i2c_driver);
}

module_init(nfc_driver_init);
module_exit(nfc_driver_exit);
#endif
#endif
