...
/*
 * PD9402AAF voice coil motor driver
 *
 *
 */
...
#define PD9402AAF_DRIVER_VERSION "v1.0.0"

#define AF_DRVNAME "PD9402AAF_DRV"
...
static int s4PD9402AAF_ReadReg(unsigned short *a_pu2Result)
{
 ...
 * I2c read/write
 ******************************************************************************/
static int PD9402AAF_WriteReg(unsigned char a_uAddr, unsigned char a_uData)
{
	int ret = 0;
...
static int PD9402AAF_ReadReg(unsigned char a_uAddr, unsigned char *a_puData)
{
	int ret = 0;
	char a_uResult;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		initdrv();
		ret = s4PD9402AAF_ReadReg(&InitPos);
...
long PD9402AAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
int PD9402AAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	LOG_INF("Start\n");
...
int PD9402AAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	pr_info("PD9402AAF driver version %s\n", PD9402AAF_DRIVER_VERSION);
...
int PD9402AAF_PowerDown(struct i2c_client *pstAF_I2Cclient, int *pAF_Opened)
{
	unsigned char data;
	...
	PD9402AAF_WriteReg(0x08, 0x0a);

	PD9402AAF_ReadReg(0x02, &data);
	data |= 0x01;
	PD9402AAF_WriteReg(0x02, data); /* enter PD mode, PD = 1; */
...
int PD9402AAF_GetFileName(unsigned char *pFileName)
{
