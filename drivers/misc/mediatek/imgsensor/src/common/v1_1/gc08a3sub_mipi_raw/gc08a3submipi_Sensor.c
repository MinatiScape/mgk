...
#endif
extern int curr_sensor_id;// prize add by zhuzhengjiang for camera 20210721 start
/*prize add by zhuzhengjiang for mtk otp 20210730 start*/
#if GC08A3SUB_OTP_FOR_MTK
#define GC08A3SUB_OTP_DATA_DUMP    0
struct imgsensor_otp_info_struct gc08a3sub_otp_info ={0};
EXPORT_SYMBOL(gc08a3sub_otp_info);
#endif
/*prize add by zhuzhengjiang for mtk otp 20210730 end*/
static DEFINE_SPINLOCK(imgsensor_drv_lock);

static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC08A3SUB_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
kal_uint16 gc08a3sub_fullsize_addr_data[] = {
	   /*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
...
kal_uint16 gc08a3sub_binning_addr_data[] = {
	/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x55,
...
static void preview_setting(void)
{
	...
	table_write_cmos_sensor(gc08a3sub_binning_addr_data,
		sizeof(gc08a3sub_binning_addr_data)/sizeof(kal_uint16));
	gc08a3sub_stream_on();
...
{
	gc08a3sub_stream_off();	
	table_write_cmos_sensor(gc08a3sub_fullsize_addr_data,
		sizeof(gc08a3sub_fullsize_addr_data)/sizeof(kal_uint16));
	gc08a3sub_stream_on();
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	if (modes){
		if (modes == 5 && (pdata != NULL)) {
			...
			write_cmos_sensor_8bit(0x008d, 0x00); // black
		}
		else
			write_cmos_sensor_8bit(0x008c, 0x01);
	}
...
/*prize add by zhuzhengjiang for mtk otp 20210730 start*/
#if GC08A3SUB_OTP_FOR_MTK
static void gc08a3sub_custom_read_otp_init(void)
{
	memset(&gc08a3sub_otp_info, 0, sizeof(struct imgsensor_otp_info_struct));
	...
	write_cmos_sensor_8bit(0x0316, 0x09);//[3] otpclk_en
	write_cmos_sensor_8bit(0x0a67, 0x80);//[7] otp_en
...
static void gc08a3sub_custom_read_otp_info(void)
{
    kal_uint32 addr = 0;
    kal_uint8 index = 0;

	//info flag
	addr = GC08A3SUB_OTP_INFO_FLAG_ADDR_OFFSET;
	write_cmos_sensor_8bit(0x0a69,(addr >> 8));
	write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
	write_cmos_sensor_8bit(0x0313,0x20);// otp mode3,[7]:otp auto start [6]:otp write [5]otp read [4:0]:reserved
	gc08a3sub_otp_info.info_flag = read_cmos_sensor(0x0a6c);  //[5:4]:grop3 [3:2]:grop2 [1:0]:group1
	if(gc08a3sub_otp_info.info_flag == 0) {
		pr_debug("error: gc08a3sub otp data is empty \n");
		return;
	}
	else if(gc08a3sub_otp_info.info_flag == 0x10) {
		index = 3;
	} 
	index  = (gc08a3sub_otp_info.info_flag / 4) + 1;
	pr_debug("gc08a3sub otp  group %d info is valid\n",gc08a3sub_otp_info.info_flag,index);
	...
	write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
	write_cmos_sensor_8bit(0x0313,0x20);// otp mode3,[7]:otp auto start [6]:otp write [5]otp read [4:0]:reserved
	gc08a3sub_otp_info.module_id = read_cmos_sensor(0x0a6c);
	...
	gc08a3sub_otp_info.year = read_cmos_sensor(0x0a6c);
	...
	write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
	write_cmos_sensor_8bit(0x0313,0x20);
	gc08a3sub_otp_info.month = read_cmos_sensor(0x0a6c);
	...
	write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
	write_cmos_sensor_8bit(0x0313,0x20);
	gc08a3sub_otp_info.day = read_cmos_sensor(0x0a6c);
	pr_debug("gc08a3sub otp module_id: 0x%x  date: %d-%d-%d \n",gc08a3sub_otp_info.module_id,gc08a3sub_otp_info.year,gc08a3sub_otp_info.month,gc08a3sub_otp_info.day);

}
static void gc08a3sub_custom_read_kgroup(kal_uint32 addr, kal_uint8 *buff, int size)
{
	kal_uint16 i;
	for (i = 0; i < size; i++) {
		write_cmos_sensor_8bit(0x0a69,(addr >> 8));
		write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
		write_cmos_sensor_8bit(0x0313,0x20);// otp mode3,[7]:otp auto start [6]:otp write [5]otp read [4:0]:reserved
		*(buff) = read_cmos_sensor(0x0a6c);
	#if GC08A3SUB_OTP_DATA_DUMP
		pr_debug("gc08a3sub otp  data: 0x%x \n",*(buff));
	#endif
		addr += 8;
		buff++;
	}
...
static void gc08a3sub_custom_get_otp_data(void)
{
	// get awb data
	kal_uint32 addr = 0;
	kal_uint8 index = 0;
	//info awb flag
	addr = GC08A3SUB_OTP_AWB_FLAG_ADDR_OFFSET;
	write_cmos_sensor_8bit(0x0a69,(addr >> 8));
	write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
	write_cmos_sensor_8bit(0x0313,0x20);//
	gc08a3sub_otp_info.awb.awb_flag = read_cmos_sensor(0x0a6c);  //[5:4]:grop3 [3:2]:grop2 [1:0]:group1
	if(gc08a3sub_otp_info.awb.awb_flag == 0) {
		pr_debug("error: gc08a3sub otp awb data is empty \n");
		return;
	}
	else if(gc08a3sub_otp_info.awb.awb_flag == 0x10) {
		index = 3;
	}
	else
		index  = (gc08a3sub_otp_info.awb.awb_flag / 4) + 1;

	addr = GC08A3SUB_OTP_AWB_GROUP_OFFSET * (index -1)  + GC08A3SUB_OTP_AWB_DATA_ADDR_OFFSET;
#if GC08A3SUB_OTP_DATA_DUMP
		pr_debug("gc08a3sub otp dump awb data\n");
#endif
	gc08a3sub_custom_read_kgroup(addr,&(gc08a3sub_otp_info.awb.unit_r_h),GC08A3SUB_OTP_AWB_DATA_SIZE);

	pr_debug("gc08a3sub otp awb unit (R/Gr/Gb/B): 0x%x 0x%x 0x%x 0x%x ,golden(R/Gr/Gb/B): 0x%x 0x%x 0x%x 0x%x \n",
		gc08a3sub_otp_info.awb.unit_r_h << 8 | gc08a3sub_otp_info.awb.unit_r_l,gc08a3sub_otp_info.awb.unit_gr_h << 8 | gc08a3sub_otp_info.awb.unit_gr_l,
		gc08a3sub_otp_info.awb.unit_gb_h << 8 | gc08a3sub_otp_info.awb.unit_gb_l,gc08a3sub_otp_info.awb.unit_b_h << 8 | gc08a3sub_otp_info.awb.unit_b_l,
		gc08a3sub_otp_info.awb.golden_r_h << 8 | gc08a3sub_otp_info.awb.golden_r_l,gc08a3sub_otp_info.awb.golden_gr_h << 8 | gc08a3sub_otp_info.awb.golden_gr_l,
		gc08a3sub_otp_info.awb.golden_gb_h << 8 | gc08a3sub_otp_info.awb.golden_gb_l,gc08a3sub_otp_info.awb.golden_b_h << 8 | gc08a3sub_otp_info.awb.golden_b_l);

	//info lsc flag
	addr = GC08A3SUB_OTP_LSC_FLAG_ADDR_OFFSET;
	write_cmos_sensor_8bit(0x0a69,(addr >> 8));
	write_cmos_sensor_8bit(0x0a6a,(addr & 0xff));
	write_cmos_sensor_8bit(0x0313,0x20);//
	gc08a3sub_otp_info.lsc_flag = read_cmos_sensor(0x0a6c);  //[5:4]:grop3 [3:2]:grop2 [1:0]:group1
	if(gc08a3sub_otp_info.lsc_flag == 0) {
		pr_debug("error: gc08a3sub otp lsc data is empty \n");
		return;
	}
	else if(gc08a3sub_otp_info.lsc_flag == 0x10) {
		index = 3;
	} 
	else
		index  = (gc08a3sub_otp_info.info_flag / 4) + 1;
	pr_debug("gc08a3sub otp lsc group %d data is valid\n",gc08a3sub_otp_info.info_flag,index);
#if GC08A3SUB_OTP_DATA_DUMP
		pr_debug("gc08a3sub otp dump lsc data\n");
#endif
	addr = GC08A3SUB_OTP_LSC_GROUP_OFFSET * (index -1)  + GC08A3SUB_OTP_LSC_DATA_ADDR_OFFSET;

	gc08a3sub_custom_read_kgroup(addr,&gc08a3sub_otp_info.lsc[0],GC08A3SUB_OTP_LSC_DATA_SIZE);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    kal_uint8 i = 0;
    kal_uint8 retry = 2;
	// prize add by zhuzhengjiang for camera 20210721 start
	if(curr_sensor_id != 1){
		*sensor_id = 0xFFFFFFFF;
		LOG_INF("current sensor idx is not match  physical idx\n");
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	// prize add by zhuzhengjiang for camera 20210721 end
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc08a3sub_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				/*prize add by zhuzhengjiang for mtk otp 20210730 start*/
			#if GC08A3SUB_OTP_FOR_MTK
				gc08a3sub_custom_read_otp_init();
				mdelay(10);
				gc08a3sub_custom_read_otp_info();
				gc08a3sub_custom_get_otp_data();
			#endif
				/*prize add by zhuzhengjiang for mtk otp 20210730 end*/
				return ERROR_NONE;
			}
			pr_err("[gc08a3sub_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() + 1;
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc08a3sub_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_err("[gc08a3sub_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
