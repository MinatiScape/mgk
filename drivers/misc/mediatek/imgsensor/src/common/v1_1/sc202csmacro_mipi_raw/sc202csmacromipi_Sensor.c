...
#include "sc202csmacromipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "sc202csmacro_camera_sensor_JF"
#define LOG_1 LOG_INF("SC202CSMACRO, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)
#define MIRROR_FLIP_ENABLE 1
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (reg_gain < SC202CSMACRO_SENSOR_GAIN_BASE)
		reg_gain = SC202CSMACRO_SENSOR_GAIN_BASE;
	else if (reg_gain > SC202CSMACRO_SENSOR_GAIN_MAX)
		reg_gain = SC202CSMACRO_SENSOR_GAIN_MAX;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	kal_uint16 SC202CSMACRO_AGC_Param[SC202CSMACRO_SENSOR_GAIN_MAX_VALID_INDEX][2] = {
		{  1024,  0x00 },
		{  2048,  0x01 },
		{  4096,  0x03 },
		{  8192,  0x07 },
	...
	for (gain_index = SC202CSMACRO_SENSOR_GAIN_MAX_VALID_INDEX - 1; gain_index >= 0; gain_index--)
		if (reg_gain >= SC202CSMACRO_AGC_Param[gain_index][0])
			break;

	write_cmos_sensor(0x3e09, SC202CSMACRO_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * SC202CSMACRO_SENSOR_GAIN_BASE / SC202CSMACRO_AGC_Param[gain_index][0];
	write_cmos_sensor(0x3e07, (temp_gain >> 3) & 0xff);
	
	LOG_INF("Exit! SC202CSMACRO_AGC_Param[gain_index][1] = 0x%x, temp_gain = 0x%x, gain = 0x%x, reg_gain = %d\n",
		SC202CSMACRO_AGC_Param[gain_index][1], temp_gain, gain, reg_gain);
...
static kal_uint32 open(void)
{
	...
	//sc202csmacro_gcore_check_version();

	/* Don't Remove!! */
	//sc202csmacro_gcore_identify_otp();

	/* initail sequence write in */
	sensor_init();

	/* write registers to sram */
	//sc202csmacro_gcore_update_otp();
...
UINT32 SC202CSMACROMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
