...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	do {
		*sensor_id = return_sensor_id();
		printk("Read sensor id: 0x%x, img sensor_id:0x%x\n",*sensor_id, imgsensor_info.sensor_id);
		if (*sensor_id == imgsensor_info.sensor_id) {
		    printk("Read ok sensor id: 0x%x, i2c_write_id:0x%x\n",*sensor_id, imgsensor.i2c_write_id);
		    return ERROR_NONE;
		}

		printk("Read sensorid fail: 0x%x\n",*sensor_id);
		retry--;
	} while (retry > 0);
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	if (modes) {
		...
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
		 ...
		 if(modes == 1 && (pdata != NULL)){
			pr_info("R=0x%x,Gr=0x%x,B=0x%x,Gb=0x%x", pdata->COLOR_R, pdata->COLOR_Gr, pdata->COLOR_B, pdata->COLOR_Gb);
			...
			write_cmos_sensor(0x0600, 0x0301); 
		 }
	} else {
		// 0x5E00[8]: 1 enable,  0 disable
		// 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK
