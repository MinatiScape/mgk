...
{
		printk("lepton_enum_fmt_vid_cap\n");
	if (f->index != 0)
		return -EINVAL;
...
{
	printk("lepton_enum_frameintervals\n");
	if (f->index != 0)
		return -EINVAL;
...
static void lepton_spi_done_callback(void *context)
{
    ...
    printk("lepton_spi_done_callback111 lepton_model=%d lep->override_frame_validation=%d subframe_is_good=%d\n",lepton_model,lep->override_frame_validation,subframe_is_good);	   
	 ...
	 */
   printk("lepton_spi_done_callback222 lep_buf=%p subframe_is_duplicate=%d subframe_is_good=%d\n",lep_buf,subframe_is_duplicate,subframe_is_good);	 
...
static void lepton_start_transfer(struct lepton *lep, void *rx_buf, dma_addr_t rx_dma, size_t rx_len)
{
   ...
   printk("lepton_start_transfer111\n");
	spin_lock_irqsave(&lep->lock, flags);
   ...
   printk("lepton_start_transfer222 lep->spi_dev->max_speed_hz=%d\n",lep->spi_xfer->speed_hz);	
//drv add by lipengpeng 20240220 start
...
#if 1
	//if (printk_ratelimit()) {
		printk("lepton_vsync_handler VSYNC %d", lep->vsync_count);
...
/*	struct device_node *node = NULL;
	...
	printk("lepton_get_gpio_dts_info %s line = %d\n", __func__, __LINE__);
	...
	if (node) {
		...
		if (pdev) {
			...
			if (IS_ERR(lepton_pinctrl)) {
				...
				printk("failed can't find lepton filr pinctrl\n");
				return ret;
			}
		} else {
			printk("failed platform device is null\n");
		}
	} else {
		printk("failed device node is null\n");
	}
*/
printk("lepton_get_gpio_dts_info %s line = %d\n", __func__, __LINE__);
	...
	if (IS_ERR(lepton_pinctrl)) {
		pr_err("Failed to get devm_pinctrl_get ret = %d\n",IS_ERR(lepton_pinctrl));
		ret = PTR_ERR(lepton_pinctrl);
		return ret;
	}	
	
    printk("of_find_compatible_node  111\n");
	
	lepton_default = pinctrl_lookup_state(lepton_pinctrl, "default");
	if (IS_ERR(lepton_default)) {
		pr_err("Failed to init default\n");
		ret = PTR_ERR(lepton_default);
	}

	//if (lepton_pinctrl) {
		pinctrl_select_state(lepton_pinctrl, lepton_default);
	//		ILI_ERR("Failed to select default pinstate\n");
	//}	
	lepton_irq_active = pinctrl_lookup_state(lepton_pinctrl, "lepton_irq");
	if (IS_ERR(lepton_irq_active)) {
		pr_err("Failed to init lepton_irq_active\n");
	...
	if (IS_ERR(lepton_reset_active)) {
		pr_err("Failed to init lepton_reset_active\n");
	...
	if (IS_ERR(lepton_reset_suspend)) {
		pr_err("Failed to init lepton_reset_suspend\n");
	...
	//	pr_err("Failed to init lepton_miso_spi_active\n");
	...
	if (IS_ERR(lepton_pdn_high_active)) {
		pr_err("Failed to init lepton_pdn_high_active\n");
	...
	if (IS_ERR(lepton_pdn_low_suspend)) {
		pr_err("Failed to init lepton_pdn_low_suspend\n");
...
//vdd3.0 vddio2.8v vddc1.2v
	...
	if (IS_ERR(lepton_vdd_high_active)) {
		pr_err("Failed to init lepton_vdd_high_active\n");
	...
	if (IS_ERR(lepton_vdd_low_suspend)) {
		pr_err("Failed to init lepton_vdd_low_suspend\n");
	...
	if (IS_ERR(lepton_vddio_high_active)) {
		pr_err("Failed to init lepton_vddio_high_active\n");
	...
	if (IS_ERR(lepton_vddio_low_suspend)) {
		pr_err("Failed to init lepton_vddio_low_suspend\n");
	...
	if (IS_ERR(lepton_vddc_high_active)) {
		pr_err("Failed to init lepton_vddc_high_active\n");
	...
	if (IS_ERR(lepton_vddc_low_suspend)) {
		pr_err("Failed to init lepton_vddc_low_suspend\n");
...
//mosi miso cs clk	
	...
	if (IS_ERR(lepton_miso_pull_active)) {
		pr_err("Failed to init lepton_miso_pullhigh\n");
	...
	if (IS_ERR(lepton_miso_pull_suspend)) {
		pr_err("Failed to init lepton_miso_pulllow\n");
	...
	if (IS_ERR(lepton_mosi_pull_active)) {
		pr_err("Failed to init lepton_mosi_pull_up\n");
	...
	if (IS_ERR(lepton_mosi_pull_suspend)) {
		pr_err("Failed to init lepton_mosi_pull_down\n");
		ret = PTR_ERR(lepton_mosi_pull_suspend);
	}
  printk("of_find_compatible_node  222\n");	
	return ret;
}
//drv add by lipengpen 20240220 end 
...
static long leptondev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
   ...
   printk("leptondev_ioctl cmd=%x START_SPI_IOCTL=%x",cmd ,START_SPI_IOCTL);  
...
/*	lep->irq_gpio = of_get_named_gpio(of_node, "lepton,irq-gpio", 0);
	if (lep->irq_gpio < 0)
		printk("lepton_irq_gpio not available\n");
    ...
    if (gpio_is_valid(lep->irq_gpio)) {
        ...
        if (lep->irq < 0) {
            printk("failed to lepton_irq_gpio\n");
            return -EINVAL;
        }
    } else {
...
*/	
	...
	//ret = request_threaded_irq(lep->irq, NULL, lepton_vsync_handler, IRQF_TRIGGER_RISING , "lepton_irq", lep);
	if (ret) {
...
//drv add by lipengpeng 20240323 start 
	...
	if (status)
	{
			printk("lepton misc_register failed status=%d",status);
	}
//drv add by lipengpeng 20240323 end 	
