...
int fts_ex_mode_switch(enum _ex_mode mode, u8 value)	//drv-modify by shenwenbin for TP charger mode 20240304
{
    ...
    case MODE_CHARGER:
        ...
        break;
	//drv-mod by shenwenbin for add game mode 20240304 start
	case MODE_GAME:
		...
		break;
	//drv-mod by shenwenbin for add game mode 20240304 end
    default:
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count += snprintf(buf + count, PAGE_SIZE, "GAME Reg(0xB8):%d\n", val);
    mutex_unlock(&input_dev->mutex);
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!ts_data->game_mode) {
            ...
            ret = fts_ex_mode_switch(MODE_GAME, ENABLE);
            if (ret >= 0) {
        ...
        if (ts_data->game_mode) {
            ...
            ret = fts_ex_mode_switch(MODE_GAME, DISABLE);
            if (ret >= 0) {
...
static struct attribute *fts_touch_mode_attrs[] = {
    ...
    &dev_attr_fts_game_mode.attr,	//drv-mod by shenwenbin for add game mode 20240304
    NULL,
};
...
//drv-mod by shenwenbin for add game mode 20240304 start
	if (ts_data->game_mode) {
        fts_ex_mode_switch(MODE_GAME, ENABLE);
    }
//drv-mod by shenwenbin for add game mode 20240304 end
...
int fts_ex_mode_init(struct fts_ts_data *ts_data)
{
    ...
    ts_data->cover_mode = DISABLE;
    ts_data->charger_mode = DISABLE;
	ts_data->game_mode = DISABLE;	//drv-mod by shenwenbin for add game mode 20240304
