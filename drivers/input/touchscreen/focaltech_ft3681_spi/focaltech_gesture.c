...
#endif
/* drv add tp gesture node end */
/******************************************************************************
* Private constant and macro definitions using #define
*****************************************************************************/
#define KEY_GESTURE                           	KEY_U
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_DOUBLECLICK:
        gesture = KEY_POWER; //modify by shenwenbin for gestures 20240129// KEY_GESTURE_U; /* drv modify double click gesture */
		if(fts_data->wakeup_click_enabled) {
...
#if FTS_FOD_EN
static void tp_double_type_func(unsigned char on)
{
    //modify by shenwenbin for gestures 20240129 start
    if (fts_data->suspended) {
        FTS_INFO("In suspend,not operation gesture mode!");
        return;
    }
    mutex_lock(&fts_data->input_dev->mutex);
    //modify by shenwenbin for gestures 20240129 end
	if(1 == on){
		fts_data->gesture_support = true;
		fts_data->wakeup_click_enabled = true;
		FTS_INFO("%s enter DOUBLE-TAP gesture\n", __func__);
	}else if(0 == on){
		fts_data->wakeup_click_enabled = false;
		if(!fts_data->finger_fod_enabled)
		fts_data->gesture_support = false;
		
		FTS_INFO("%s close DOUBLE-TAP gesture\n", __func__);
	}
    mutex_unlock(&fts_data->input_dev->mutex);  //modify by shenwenbin for gestures 20240129
}
static void tp_double_fod_func(unsigned char on)
{
	if(fts_data->suspended) {
		FTS_ERROR("tp is suspend, fod set fail\n", __func__);
		return;
	}
	if(1 == on){
		fts_data->gesture_support = true;
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
	...
	/* drv add tp gesture function start */
    input_set_capability(input_dev, EV_KEY, KEY_WAKEUP);
    input_set_capability(input_dev, EV_KEY, KEY_GESTURE);
	/* drv add tp gesture function end */
	...
	/* drv add tp gesture function start */
    __set_bit(KEY_POWER, input_dev->keybit);    //modify by shenwenbin for gestures 20240129
    ...
    ts_data->gesture_support = 0; // FTS_GESTURE_EN;  /* drv modify tp gesture */
...
/* drv add tp gesture node start */	
#if IS_ENABLED(CONFIG_PRIZE_COMMON_NODE)
		prize_common_node_register("GESTURE", &tp_double_type_func);
#if FTS_FOD_EN
		prize_common_node_register("FINGER", &tp_double_fod_func);
		atomic_set(&fts_data->fod_figer_state, 0);
//		atomic_set(&fts_data->now_system_status, 1);
#endif
#endif
/* drv add tp gesture node end */
