...
/* fae, drv modify fod start */
#if FTS_FOD_EN
static int fts_fod_recovery(struct fts_ts_data *ts_data);
#endif
/* fae, drv modify fod end */

/*prize modify fod function 20230218 start*/
#define KEY_FOD_DOWN							KEY_F24
#define KEY_FOD_UP								KEY_F23
extern bool panel_visionox_fod_is_enabled(void);   /* drv modify K80NF-263, K80NF-273  */
/*prize modify fod function 20230218 end*/
...
void fts_fod_enable(int enable)
{
	...
	atomic_set(&ts_data->fod_figer_state, 0);
    if (enable) {
        FTS_INFO("Fod enable");
        ts_data->finger_fod_enabled = ENABLE;
        fts_fod_set_reg(FTS_VAL_FOD_ENABLE);
    } else {
        FTS_INFO("Fod disable");
        ts_data->finger_fod_enabled = DISABLE;
...
static int fts_fod_readdata(struct fts_ts_data *ts_data)
{
	...
	FTS_DEBUG("fod_val[1]: %x: fod_figer_state=%d",fod_val[1],atomic_read(&ts_data->fod_figer_state));
    if (fod_val[1] == 0x26 && (panel_visionox_fod_is_enabled() || (atomic_read(&ts_data->fod_figer_state) == 1))){
        ...
        if (fod_down && (atomic_read(&ts_data->fod_figer_state) == 0)) {
            /* FOD down, need do something to tell host */
            atomic_set(&ts_data->fod_figer_state, 1);
...
//        FTS_DEBUG("not fod data, don't process fod");
        ret = 0;
    }
...
{
    return (ts_data->finger_fod_enabled && (atomic_read(&ts_data->fod_figer_state) == 1));
}
...
//    atomic_set(&ts_data->fod_figer_state, 0);
	if(fts_fod_checkdown(ts_data)) {
		queue_delayed_work(ts_data->ts_workqueue, &ts_data->fodcheck_work, msecs_to_jiffies(1000));
	}
/* drv modify K80NF-263, K80NF-273 end */	
    return 0;
}
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            if ((ts_data->log_level >= 2) ||
                ...
                FTS_DEBUG("[B]P%d(%d, %d)[p:%d,tm:%d,%d] DOWN!",
                          events[i].id, events[i].x, events[i].y,
                          events[i].p, events[i].area, events[i].minor);
            }
        } else {
            input_mt_slot(input_dev, events[i].id);
            input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, false);
...
static int fts_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	if (ts_data->psy_name && !strcmp(ts_data->psy_name, "battery")) {
	...
	if (!psy) {
		FTS_ERROR("Couldn't get psy\n");
		return -EINVAL;
	}
	if (!strcmp(psy->desc->name, "usb") || !strcmp(psy->desc->name, "battery")) {
		if (psy && val == POWER_SUPPLY_PROP_STATUS) {
			if (isPsyBattery) {
				//FTS_DEBUG("fts_charger_mode_notifier_callback event psy->desc->name is battery");
			...
			if (ret < 0) {
				FTS_ERROR("Couldn't get POWER_SUPPLY_PROP_STATUS rc=%d\n", ret);
				return ret;
			} else {
				if (ts_data->charger_status != prop.intval) {
					FTS_DEBUG("usb prop.intval = %d;ts_data->charger_status = %d;ts_data->charger_mode = %d", prop.intval,ts_data->charger_status,ts_data->charger_mode);
					ts_data->charger_status = prop.intval;

					switch (ts_data->charger_status){
					...
					if (!ts_data->suspended && (ts_data->fw_loading == 0)) {
			            FTS_DEBUG("enter charger mode swith");
			            ret = fts_ex_mode_switch(MODE_CHARGER, ts_data->charger_mode);
			            if (ret >= 0) {
			                FTS_DEBUG("switch charger mode successfully");
			            }
        			}
				}
			}
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            gpio_direction_output(ts_data->pdata->reset_gpio, 1);   //drv-modify by shenwenbin for suspend HIGH RST 20240402
            msleep(1);
...
static int fts_power_source_suspend(struct fts_ts_data *ts_data)
{
	...
	//drv-temporary modify by shenwenbin for VIO18 OCP reboot 20231111 start 
    //ret = fts_power_source_ctrl(ts_data, DISABLE);
	ret = fts_power_source_ctrl(ts_data, ENABLE);
	//drv-temporary modify by shenwenbin for VIO18 OCP reboot 20231111 end 
    if (ret < 0) {
...
//drv-mod by shenwebin for AOD show time TP can't double-wakeup 20240323 start
#if IS_ENABLED(CONFIG_PRIZE_OLED_NODE)
extern bool mtk_panel_doze_state(void);
#endif
//drv-mod by shenwebin for AOD show time TP can't double-wakeup 20240323 end
static int fts_ts_resume(struct device *dev)
{
    struct fts_ts_data *ts_data = fts_data;
...
//drv-mod by shenwebin for AOD show time TP can't double-wakeup 20240323 start
#if IS_ENABLED(CONFIG_PRIZE_OLED_NODE)
	 if(mtk_panel_doze_state()){
	 	FTS_DEBUG("Panel enter doze mode,ts_data->suspended = %d,mtk_panel_doze_state = %d",ts_data->suspended,mtk_panel_doze_state());
        return 0;
	 }
#endif
//drv-mod by shenwebin for AOD show time TP can't double-wakeup 20240323 start
...
/* drv modify fod function 20230218 start*/
static void fodcheck_func(struct work_struct *work)
{
    struct fts_ts_data *ts_data = container_of(work, struct fts_ts_data, fodcheck_work.work);
	struct input_dev *input_dev = ts_data->input_dev;
	if(fts_fod_checkdown(ts_data)) {
	    /* FOD up, need do something to tell host */
        atomic_set(&ts_data->fod_figer_state, 0);
		...
		input_report_key(input_dev, KEY_FOD_UP, 0);
		input_sync(input_dev);
		FTS_INFO("resume report fod up");
	}
}
/* drv modify fod function 20230218 end*/
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->ts_workqueue) {
		...
		INIT_DELAYED_WORK(&ts_data->fodcheck_work, fodcheck_func); /* drv modify fod function 20230218 */
    }
...
//drv-modify by shenwenbin for TP charger mode 20240304 end
    ...
    if (ts_data->ts_workqueue) destroy_workqueue(ts_data->ts_workqueue);
	if (ts_data->ts_workqueue) cancel_delayed_work_sync(&ts_data->fodcheck_work); /* drv modify fod function 20230218 */
