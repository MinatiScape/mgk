 ...
 */
#define FTS_PATCH_COMERR_PM                 1		//drv modify by shenwenbin for double tap wakeup function
...
struct fts_ts_data {
    ...
    bool prc_mode;
    bool esd_support;
	bool game_mode;	//drv-mod by shenwenbin for add game mode 20240304
...
//drv-modify by shenwenbin for TP charger mode 20240304 end
	...
	bool wakeup_click_enabled;  // drv add 1-enable   0-disable
#if FTS_FOD_EN
	bool finger_fod_enabled;    // drv add fod 1-down   0-up
	atomic_t fod_figer_state;     //drv add 1-down   0-up
    struct delayed_work fodcheck_work; // drv add to check fod up
#endif
    u8 pen_etype;
...
enum _ex_mode {
    ...
    MODE_COVER,
    MODE_CHARGER,
    MODE_GAME,	//drv-mod by shenwenbin for add game mode 20240304
};
//drv-modify by shenwenbin for TP charger mode 20240304 end
...
int fts_ex_mode_switch(enum _ex_mode mode, u8 value);	//drv-modify by shenwenbin for TP charger mode 20240304
