...
/* drv added by chenjiaxi, touch data reporting contrl, start */
bool report_status = true;
/* drv added by chenjiaxi, touch data reporting contrl, end */
...
static ssize_t report_status_control_read(struct file *file, char __user *buffer, size_t count, loff_t *offset)
{
	char value = report_status;

	// Copy the value to the user space buffer
	if (copy_to_user(buffer, &value, sizeof(char)) != 0) {
		return -EFAULT;
	}
...
static ssize_t report_status_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *offset)
{
	char value;
	...
	ILI_INFO("%s: report_status = %d.\n", __func__, report_status);
	return sizeof(char);
}

static const struct proc_ops report_status_control_ops = {
	.proc_read = report_status_control_read,
...
/* drv added by chenjiaxi, touch data reporting contrl, start */
	...
	if (touch_proc_dir == NULL) {
		ILI_INFO("%s: touch_proc_dir file create failed!\n", __func__);
	}
	...
	if (report_status_control_file == NULL) {
		ILI_INFO("%s: proc report control file create failed!\n", __func__);
