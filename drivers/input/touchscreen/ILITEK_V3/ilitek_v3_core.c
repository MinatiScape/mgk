...
//prize add by lipengpeng 20220106 start 
extern int BOOT_FW_ILI9882Q;
//prize add by lipengpeng 20220106 end 
...
//prize add by lipengpeng 20220106 end 
   ...
   printk("lpp---BOOT_FW_ILI9882Q=%d \n",BOOT_FW_ILI9882Q);
//prize add by lipengpeng 20220106 start 
//#if (BOOT_FW_UPDATE | HOST_DOWN_LOAD )
if(1==BOOT_FW_ILI9882Q){
		fw_boot_th = kthread_run(ili_fw_upgrade_handler, NULL, "ili_fw_boot");
		printk("lpp---open boot fw start \n");
...
//#else
	...
	ILI_INFO("Registre touch to input subsystem\n");
	//prize  hjw  for null pointer start
	//ili_input_register();   
	if(ili_input_register() < 0)  //prize  hjw
	{
		return -ENODEV;
	}
	//prize  hjw  for null pointer end
	ili_wq_ctrl(WQ_ESD, ENABLE);
	ili_wq_ctrl(WQ_BAT, ENABLE);
	ilits->boot = true;
 }
