...
int ili_get_tp_recore_data(bool mcu)
{
	...
	u32 base_addr = 0x20000, addr, len, i, fcnt;// *ptr,
...
static struct dentry *proc_dir_ilitek;
...
void ili_node_init(void)
{
	...
	proc_dir_ilitek = debugfs_create_dir("ilitek", NULL);

	for (; i < ARRAY_SIZE(iliproc); i++) {
		iliproc[i].node = debugfs_create_file(iliproc[i].name, 0644, proc_dir_ilitek, NULL, iliproc[i].fops);
