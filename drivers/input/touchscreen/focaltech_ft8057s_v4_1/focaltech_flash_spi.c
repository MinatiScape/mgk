...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    if (ret < 0) {
        ...
        FTS_INFO("read chip id:0x%02x", chip_id);
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	    ret = fts_read_reg(FTS_REG_FW_VER, &chip_id);
	    if ((ret < 0) || (chip_id == 0xFF) || (chip_id == 0x00)){
	        strcpy(current_tp_info.chip, "FT8057S,Firmware version:get tp fw version fail!\n");
        }
	    else{
	        sprintf(current_tp_info.chip, "FT8057S,Firmware version:0x%02x",chip_id);
        }
		...
		sprintf(current_tp_info.more, "%d*%d",1612,720);
#endif
    }
}
