...
EXPORT_SYMBOL_GPL(ili_tp_reset);

void ili_tp_rst_low(void)
{
	ILI_INFO("ili_tp_rst_low\n");
	gpio_direction_output(ilits->tp_rst, 1);
	mdelay(1);
	gpio_set_value(ilits->tp_rst, 0);
	mdelay(5);
}
EXPORT_SYMBOL_GPL(ili_tp_rst_low);

void ili_tp_rst_high(void)
{
	ILI_INFO("ili_tp_rst_high\n");
	gpio_direction_output(ilits->tp_rst, 1);
	mdelay(1);
	gpio_set_value(ilits->tp_rst, 1);
	mdelay(5);
}
EXPORT_SYMBOL_GPL(ili_tp_rst_high);
...
static int ilitek_drm_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	int *evdata = (int *)data;

	if (data) {
		printk("ilitek_drm_notifier_callback IN");
		...
		printk("ilitek_drm_notifier_callback OUT");
	} else {
		printk("ilitek touch IC can not suspend or resume");
		return -1;
	}
...
static void ilitek_plat_sleep_init(void)
{
	...
	//if (active_panel) {
		if (mtk_disp_notifier_register("ilits Touch", &ilits->notifier_fb))
		ILI_ERR("[DRM]mtk_disp_notifier_register fail\n");
	//}
#else
...
#endif
#if IS_ENABLED(CONFIG_DRM_MEDIATEK)
	ilits->fb_notif.notifier_call = ilitek_drm_notifier_callback;

		if (mtk_disp_notifier_register("ilits Touch", &ilits->fb_notif))
		ILI_ERR("[DRM]mtk_disp_notifier_register fail\n");
#endif
...
static int ilitek_plat_probe(void)
{
	...
	/* drv added by chenjiaxi, improve spi driving capability, start */
	ilits->pin_spi_pinctrl = devm_pinctrl_get(ilits->dev);
	if (IS_ERR_OR_NULL(ilits->pin_spi_pinctrl)) {
		ILI_ERR("devm_pinctrl_get fail\n");
		ilits->pin_spi_pinctrl = NULL;
	}
	
	ilits->ctp_pin_spi_mode_default = pinctrl_lookup_state(ilits->pin_spi_pinctrl, "ctp_pin_spi_mode_default");
	if (IS_ERR_OR_NULL(ilits->ctp_pin_spi_mode_default)) {
		ILI_ERR("Failed to get pinctrl state:ctp_pin_spi_mode_default\n");
		ilits->ctp_pin_spi_mode_default = NULL;
	}

	if (ilits->pin_spi_pinctrl) {
		if (pinctrl_select_state(ilits->pin_spi_pinctrl, ilits->ctp_pin_spi_mode_default) < 0)
			ILI_ERR("Failed to select default pinstate\n");
		else
			ILI_ERR("Success to select default pinstate\n");
	}
	/* drv added by chenjiaxi, improve spi driving capability, start */
