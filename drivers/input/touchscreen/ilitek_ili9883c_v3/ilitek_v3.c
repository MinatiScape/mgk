...
//drv-mod by shenwenbin for add spi cs ctrl 20240711 start
struct pinctrl          *spi_cs_pinctrl;
struct pinctrl_state *spi_cs_pins_suspend;
struct pinctrl_state *spi_cs_pins_resume;
//drv-mod by shenwenbin for add spi cs ctrl 20240711 end
...
int ili_sleep_handler(int mode)
{
	...
	case TP_SUSPEND:
		...
		//drv-mod by shenwenbin for add spi cs ctrl 20240711 start
            if((spi_cs_pinctrl != NULL) && (spi_cs_pins_suspend != NULL)){
                ILI_INFO(" make TP spi cs into suspend");
                ret = pinctrl_select_state(spi_cs_pinctrl, spi_cs_pins_suspend);
                if (ret < 0) {
                    ILI_ERR("Set spi cs suspend pin state error:%d", ret);
                }
            }
		//drv-mod by shenwenbin for add spi cs ctrl 20240711 end
		}
	...
	case TP_RESUME:
		//drv-mod by shenwenbin for add spi cs ctrl 20240711 start
		if((spi_cs_pinctrl != NULL) && (spi_cs_pins_resume != NULL)){
            ILI_INFO("make TP spi cs into resume");
            ret = pinctrl_select_state(spi_cs_pinctrl, spi_cs_pins_resume);
            if (ret < 0) {
                ILI_ERR("Set spi cs suspend pin state error:%d", ret);
            }
        }
...
//drv-mod by shenwenbin for add spi cs ctrl 20240711 start
static int ili_spi_parse_dt(struct device *dev)
{
    int ret;

    ILI_INFO("[%s] Enter !\n", __func__);

    spi_cs_pinctrl = devm_pinctrl_get(dev);
    if (IS_ERR(spi_cs_pinctrl)) {
        ret = PTR_ERR(spi_cs_pinctrl);
        ILI_ERR("Cannot find touch pc!\n");
        goto err_out;
    }

    spi_cs_pins_suspend = pinctrl_lookup_state(spi_cs_pinctrl, "spi_cs_suspend");
    if (IS_ERR(spi_cs_pins_suspend)) {
        ret = PTR_ERR(spi_cs_pins_suspend);
        ILI_ERR("Cannot find touch pinctrl spi_cs_suspend!");
        goto err_out;
    }

    spi_cs_pins_resume = pinctrl_lookup_state(spi_cs_pinctrl, "spi_cs_resume");
    if (IS_ERR(spi_cs_pins_resume)) {
        ret = PTR_ERR(spi_cs_pins_resume);
        ILI_ERR("Cannot find touch pinctrl spi_cs_pins_resume!");
        goto err_out;
    }
	ILI_INFO("[%s] Exit !\n", __func__);
    return 0;
