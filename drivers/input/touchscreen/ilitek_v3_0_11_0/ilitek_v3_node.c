...
/* drv added by chenjiaxi, wakeup gesture, start */
/* change based on TP driver start */
struct ilitek_ts_data *g_drv_common_node_data = NULL;

bool ilitek_is_gesture_wakeup_enabled(void)
{

	if (g_drv_common_node_data == NULL)
		return false;

	if (ilits->prepared == false)
		return false;
    ...
    return g_drv_common_node_data->gesture_wakeup_enabled;
}
EXPORT_SYMBOL_GPL(ilitek_is_gesture_wakeup_enabled);

static void drv_gesture_enable(unsigned char on)
{
	if (1 == on) {
		ilits->gesture = true;
		g_drv_common_node_data->gesture_wakeup_enabled = true;
		ILI_INFO("open DOUBLE-TAP gesture\n");
	} else if (0 == on) {
		ilits->gesture = false;
		g_drv_common_node_data->gesture_wakeup_enabled = false;
		ILI_INFO("close DOUBLE-TAP gesture\n");
	}
}
/* change based on TP driver end */

static ssize_t drv_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    count = snprintf(buf, PAGE_SIZE, "Gesture Mode:%s\n", g_drv_common_node_data->gesture_wakeup_enabled ? "On" : "Off");
    return count;
}

static ssize_t drv_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    if (buf[0] == '1') {
		drv_gesture_enable(1);
		pr_err("%s set gesture_wakeup_enabled = %d\n", __func__, g_drv_common_node_data->gesture_wakeup_enabled);
    } else if (buf[0] == '0') {
		drv_gesture_enable(0);
		pr_err("%s set gesture_wakeup_enabled = %d\n", __func__, g_drv_common_node_data->gesture_wakeup_enabled);
    }

    return count;
}
static DEVICE_ATTR(gesture, S_IRUGO | S_IWUSR, drv_gesture_show, drv_gesture_store);

static int drv_gesture_probe(struct platform_device *pdev)
{
	int err = 0;
	
	platform_set_drvdata(pdev, g_drv_common_node_data);
	err = device_create_file(&pdev->dev, &dev_attr_gesture);
	if (err) {
		pr_err("%s device_create_file gesture fail", __func__);
		return -ENODEV;
	}
...
static const struct of_device_id drv_gesture_of_match[] = {
	{.compatible = "mediatek,gesture",},
	{},
};

static struct platform_driver drv_gesture_driver = {
	.probe = drv_gesture_probe,
	.driver = {
		.name = "common_node",
		.of_match_table = drv_gesture_of_match,
	},
};

int drv_gesture_init(void)
{
	return platform_driver_register(&drv_gesture_driver);
}
/* drv added by chenjiaxi, wakeup gesture, end */
...
static void drv_glove_mode_enable(unsigned char on)
{
	if (1 == on) {
		...
		g_drv_common_node_data->glove_mode_enable = true;
	} else if (0 == on) {
		ili_ic_func_ctrl_export("glove", 0x00);
		g_drv_common_node_data->glove_mode_enable = false;
	}
}
/* change based on TP driver end */
...
{
    int count = 0;
    count = snprintf(buf, PAGE_SIZE, "Glove Mode:%s\n", g_drv_common_node_data->glove_mode_enable ? "On" : "Off");
    return count;
}
...
int drv_glove_init(void)
{
    ...
    err = sysfs_create_link(drv_glove, &g_drv_common_node_data->spi->dev.kobj, "common_node"); //&g_drv_common_node_data->i2c->dev.kobj
    if (err) {
        pr_err("sysfs_create_link fail\n");
        return -1;
	}

    if (sysfs_create_file(&g_drv_common_node_data->spi->dev.kobj, &dev_attr_state.attr)) { //&g_drv_common_node_data->i2c->dev.kobj
        return -1;
	}
