...
static void ili_resume_work(struct work_struct *work)
{
	ILI_INFO("Enter\n");
	ili_config_spi(0);
	gpio_set_value(ilits->tp_rst, 1);
...
static int ili_ts_suspend(struct device *dev)
{
	ILI_INFO("Enter\n");
	if (ili_sleep_handler(TP_DEEP_SLEEP) < 0)
		ILI_ERR("TP suspend failed\n");
...
{
	int *evdata = (int *)data;
  struct ilitek_ts_data *ts_data = container_of(self, struct ilitek_ts_data,
                                  fb_notif);
	...
	if (ts_data && data) {
		...
		ILI_INFO("Ilitek touch IC can not suspend or resume");
		return -1;
	}
	return 0;
}
...
static void ilitek_plat_sleep_init(void)
{
	...
	if(ilits->ts_workqueue) {
		INIT_WORK(&ilits->resume_work, ili_resume_work);
	}
	ilits->fb_notif.notifier_call = fb_notifier_callback;
	ret = mtk_disp_notifier_register("ilitek Touch", &ilits->fb_notif);
	if (ret)
...
#if EAR_PHONE_NOTIFIER_CALLBACK
/* prize add by zhanghuimin for SN339D-1757 start */
struct blocking_notifier_head ili_earphone_notifier_chain;
EXPORT_SYMBOL_GPL(ili_earphone_notifier_chain);

int ili_earphone_notifier_register(struct notifier_block *nb)
{
	return blocking_notifier_chain_register(&ili_earphone_notifier_chain, nb);
}

void ili_earphone_notifier_unregister(struct notifier_block *nb)
{
	blocking_notifier_chain_unregister(&ili_earphone_notifier_chain, nb);
}

int ili_earphone_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	static int earphone_prsent = 0;
        int ret = 0;
	ILI_INFO("earphone notification %lu, prsent:%d\n", event, earphone_prsent);
	if (event != earphone_prsent) {
		earphone_prsent = event;
		ret = ili_ic_func_ctrl("ear_phone", earphone_prsent);
		if (ret < 0) {
			ILI_ERR("Write plug in failed\n");
		}
	} else {
		ILI_INFO("earphone_prsent =%d not change!\n", earphone_prsent);
	}
	return NOTIFY_OK;
}
struct notifier_block ili_earphone_notifier_block = {
	.notifier_call = ili_earphone_notifier_callback,
};
...
#elif SUSPEND_RESUME_SUPPORT
  ilits->ts_workqueue = create_singlethread_workqueue("ili_wq");
  if (!ilits->ts_workqueue) {
     ILI_INFO("create ili workqueue fail");
  }
...
#if EAR_PHONE_NOTIFIER_CALLBACK
/* prize add by zhanghuimin for SN339D-1757 start */
	if (ili_earphone_notifier_register(&ili_earphone_notifier_block)) {
		ILI_ERR("Failed to register ili_earphone_notifier_register\n");
	}
/* prize add by zhanghuimin for SN339D-1757 end */
#endif
...
#if EAR_PHONE_NOTIFIER_CALLBACK
/* prize add by zhanghuimin for SN339D-1757 start */  
        ili_earphone_notifier_unregister(&ili_earphone_notifier_block);
/* prize add by zhanghuimin for SN339D-1757 end */
#endif
