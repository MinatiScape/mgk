...
/* drv modify tp firmware start */
//#define FW_BIN_NAME "C60R_HKC_GC7202H_0x060C.0x0402_20230802.bin"
//#define FW_BIN_NAME "C56_HKC6.517_GC7202H_0x060C.0x0402_20231009.bin"
//#define FW_BIN_NAME "C56_Text0x0.bin"

/* drv modify tp firmware end */
...
int gcore_dump_fw_to_file(void)
{
	...
	return 0;
#else
    GTP_ERROR("not support vfs_read to get fw file");
    return -EINVAL;
#endif
}
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
	...
	sprintf(current_tp_info.chip, "gcore,fw_version:%02x%02x%02x%02x",gdev_fwu->fw_ver_in_bin[1],gdev_fwu->fw_ver_in_bin[0],gdev_fwu->fw_ver_in_bin[3],gdev_fwu->fw_ver_in_bin[2]);
#endif
...
/* DRV added by kuangliangjun, glove mode, start */
static ssize_t gcore_glove_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;

    count = snprintf(buf, PAGE_SIZE, "Glove Mode:%s\n",
                     gdev_fwu->glove_enable ? "On" : "Off");

    return count;
}

static ssize_t gcore_glove_mode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    //int ret = 0;
    ...
    if (buf[0] == '1') {
        GTP_DEBUG("enable glove mode");
		gcore_fw_event_notify(FW_GLOVE_ENABLE);
		gdev_fwu->glove_enable = true;
    } else if (buf[0] == '0') {
        GTP_DEBUG("disable glove mode");
		gcore_fw_event_notify(FW_GLOVE_DISABLE);
		gdev_fwu->glove_enable = false;
...
/* sysfs glove node
 * read example: cat gcore_glove_mode        ---read  glove mode
 * write example:echo 1 > gcore_glove_mode   ---write glove mode to 01
 */
static DEVICE_ATTR(state, S_IRUGO | S_IWUSR,
                   gcore_glove_mode_show, gcore_glove_mode_store);

/* DRV added by kuangliangjun, glove mode, end */
int gcore_create_attribute(struct device *dev)
{
	...
	err = sysfs_create_link(drv_glove, &gdev_fwu->bus_device->dev.kobj, "common_node");
	if (err) {
		GTP_ERROR("prize gcore sysfs_create_link fail\n");
		return err;
	}
	if (sysfs_create_file(&gdev_fwu->bus_device->dev.kobj, &dev_attr_state.attr)) {
		return err;
	}
	/* DRV added by kuangliangjun, glove mode, end */
