...
//drv modify by kuangliangjun for judge charger mode 20241126 start
static int gcore_charger_parse_dt(struct device_node *dev)
{
	struct device_node *boot_node = NULL;
	struct tag_bootmode *tag = NULL;

	boot_node = of_parse_phandle(dev, "bootmode", 0);
	if (!boot_node)
...
int gcore_input_device_init(struct gcore_dev *gdev)
{
	...
	set_bit(KEY_F3, gdev->input_device->keybit);
	set_bit(KEY_F4, gdev->input_device->keybit);
#endif
...
void gcore_proximity_report_to_application(void){
	...
	if(gcore_tpd_proximity_flag_off != gcore_tpd_proximity_flag_off_backup
		...
		if(gcore_tpd_proximity_flag_off){
			...
			input_report_key(gdev_intf->input_device, KEY_F3, 1);
			input_sync(gdev_intf->input_device);
			input_report_key(gdev_intf->input_device, KEY_F3, 0);
			input_sync(gdev_intf->input_device); 
			
		}else{
			input_report_abs(gdev_intf->input_ps_device, ABS_DISTANCE, 1);
			GTP_DEBUG("input_report ABS_DISTANCE 1.");
			input_report_key(gdev_intf->input_device, KEY_F4, 1);
			input_sync(gdev_intf->input_device);
			input_report_key(gdev_intf->input_device, KEY_F4, 0);
			input_sync(gdev_intf->input_device); 			
		}
...
//fenggg add compatible gc7202h/gc7272
u16 g_chip_type = 0x7203;
int gcore_probe_device(void)
{
	...
	while (retries++ < 3) {
		...
		if ((id != 0)&&((id & 0xff)!=0xff)) {
			g_chip_type = id;
			GTP_ERROR("GC%04x chip id match success,chip_type=%04x,g_chip_type=%04x",id,chip_type, g_chip_type);
			ret = 0;
			break;
		} else {
...
//	strcpy(current_tp_info.chip, "gcore,fw_version:0x0C");
	sprintf(current_tp_info.id, "0x%x",id);
	strcpy(current_tp_info.vendor, "longyu");
	sprintf(current_tp_info.more, "%d*%d", 720, 1440);
#endif
...
//drv modify by kuangliangjun for judge charger mode 20241126 start
	...
	struct device_node *node = NULL;
	node = of_find_matching_node(node, tpd_of_match);
//drv modify by kuangliangjun for judge charger mode 20241126 e	
...
//drv modify by kuangliangjun for judge charger mode 20241126 start	
	...
	val = gcore_charger_parse_dt(node);
	GTP_DEBUG("gcore_touch_probe val = %d\n", val);
	if (val == 8 || val ==9)
		return -ENODEV;
	}
//drv modify by kuangliangjun for judge charger mode 20241126 end	
	...
	gdev->disp_notifier.notifier_call = gcore_ts_fb_notifier_callback;
	if ( mtk_disp_notifier_register("gcore Touch", &gdev->disp_notifier)) {
...
void gcore_spi_config(void)
{
	...
	gcore_spi_slave->cs_setup.value = 1;
	gcore_spi_slave->cs_setup.unit = 0;
	gcore_spi_slave->cs_hold.value = 1;
	gcore_spi_slave->cs_hold.unit = 0;
	gcore_spi_slave->cs_inactive.value = 1;
	gcore_spi_slave->cs_inactive.unit = 0;
