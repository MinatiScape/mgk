...
/*mod-drv by kuangliangjun for TP gesture func 20241023 start*/
...
#endif
#endif
/*mod-drv by kuangliangjun for TP gesture func 20241023 end*/

#define GCORE_PLS_FILE "gcore_pls"
struct proc_dir_entry *gcore_pls_entry;
...
#ifdef CONFIG_ENABLE_PROXIMITY_TP_SCREEN_OFF
	gcore_pls_entry = proc_create(GCORE_PLS_FILE, 0666, NULL, &tp_ps_fops);
	if (gcore_pls_entry == NULL) {
		GTP_ERROR("create pls entry gcore_pls failed");
		return -EPERM;
	}
#endif
	return 0;
...
#ifdef CONFIG_ENABLE_PROXIMITY_TP_SCREEN_OFF
	if (gcore_pls_entry != NULL) {
		remove_proc_entry(GCORE_PLS_FILE, NULL);
	}	
#endif	
}
...
/*static const struct proc_ops gcore_tool_fops = {
	...
	.proc_compat_ioctl	= gcore_tool_ioctl,
...
static void tp_double_type_func(unsigned char on)
{
    if (gdev_fwu->ts_stat == TS_SUSPEND) {
        GTP_DEBUG("In suspend,not operation gesture mode!");
        return;
    }
	...
	if(1 == on){
		...
		g_tp_gesture_flag = 1;
		GTP_DEBUG("%s enter DOUBLE-TAP gesture\n", __func__);
	}else if(0 == on){
		gdev_fwu->gesture_wakeup_en = false;
		g_tp_gesture_flag = 0;
		GTP_DEBUG("%s close DOUBLE-TAP gesture\n", __func__);
	}
}
#endif
#endif
