...
#define MODULE_NAME "isee :fortsense-sf_hw"
...
static int sf_spi_clock_enable(bool on)
{
    ...
    if (on && (count == 0)) {
        ...
        count = 1;
		xprintk(SF_LOG_LEVEL, "%s(..) mt_spi_enable_master_clk enter.\n", __FUNCTION__);
    }
    else if ((count > 0) && (on == 0)) {
        mt_spi_disable_master_clk(sf_ctl_dev->pdev);
        count = 0;
		xprintk(SF_LOG_LEVEL, "%s(..) mt_spi_disable_master_clk enter.\n", __FUNCTION__);
    }
...
static int sf_ctl_device_power_by_regulator(bool on)
{
    ...
    if (err) {
        xprintk(KERN_ERR, "Regulator set load failed err = %d\n", err);
    }
...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
    ...
    xprintk(SF_LOG_LEVEL, "irq number is %d.\n", ctl_dev->irq_num);
    {
        int i = 0;
        ...
        for (i = SF_PIN_STATE_INT_SET ; i < SF_PIN_STATE_MAX; ++i) {
            ...
            xprintk(SF_LOG_LEVEL, "pinctrl_select_state(%s) ok.\n", sf_pinctrl_state_names[i]);
        }
    }
#else
