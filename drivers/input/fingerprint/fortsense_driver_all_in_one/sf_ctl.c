...
#endif
//prize add by lipengpeng 20220630 start
#include "../../../tee/teei/500/tz_driver/include/teei_fp.h"
//prize add by lipengpeng 20220630 end
...
static struct of_device_id  sf_of_match[] = {
   // { .compatible = COMPATIBLE_SW_FP, },
	{ .compatible = "prize,fortsense_fp", }, //clw add
    {},
};
...
// #ifdef CONFIG_PM_WAKELOCKS
	// sf_ctl_dev.wakelock = wakeup_source_create("sf_wakelock");
// #else
...
extern bool pri_mas_finger;  //add by drv
...
static int __init sf_ctl_driver_init(void)
{
	...
	//add by drv +++++++
	if(pri_mas_finger){
	    xprintk(KERN_INFO, "'%s',mas fingerprint was loaded ,so fortsense driver exit !!! \n", __FUNCTION__);
		return 0; 	
	}
	//add by drv -------
#if SF_BEANPOD_COMPATIBLE_V1
...
late_initcall_sync(sf_ctl_driver_init); 
