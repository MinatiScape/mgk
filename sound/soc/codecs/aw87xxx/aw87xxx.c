				...
				struct device_node *dev_node)
{
	...
	ret = of_get_named_gpio(dev_node, "gpio_hac_en", 0);
	if (ret < 0) {
		AW_DEV_LOGI(aw87xxx->dev, "gpio_hac_en is gpio provided, HAC reset unavailable");
		//aw87xxx->aw_dev.rst_gpio = AW_NO_RESET_GPIO;
		//aw87xxx->aw_dev.hwen_status = AW_DEV_HWEN_INVALID;
	} else {
	    HAC_GPIO = ret;
		AW_DEV_LOGI(aw87xxx->dev, "hac gpio[%d] parse succeed", ret);
		if (gpio_is_valid(HAC_GPIO)) {
			ret = devm_gpio_request_one(aw87xxx->dev,
					HAC_GPIO,
					GPIOF_OUT_INIT_LOW, "gpio_hac_en");
			if (ret < 0) {
				AW_DEV_LOGE(aw87xxx->dev, "HAC- request failed");
				return ret;
			}
		}
	}
